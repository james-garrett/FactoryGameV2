...
public class CityBuilderEditorWindow : OdinMenuEditorWindow
    {
        [MenuItem("Tools/City Builder Editor")]
        private static void Open()
        {
            var window = GetWindow<CityBuilderEditorWindow>();
            window.position = GUIHelper.GetEditorWindowRect().AlignCenter(800, 500);
        }

        public string machinesPath = "Assets/Factory Builder Template/Prefabs/Resources/Machines/";
        public string scriptableObjectsPath = "Assets/Factory Builder Template/Scriptable Objects/";
    ...
    protected override OdinMenuTree BuildMenuTree()
        {
            ...
            //tree.Add("Machines", new CharacterTable(CharacterOverview.Instance.AllCharacters));

            //tree.Add("Scriptable Objects", new CharacterTable(CharacterOverview.Instance.AllCharacters));

            // Adds all machines.
            tree.AddAllAssetsAtPath("Machines", $"{machinesPath}", typeof(Machine), true, true);

            // Add all scriptable object items.
            tree.AddAllAssetsAtPath("Scriptable Objects", $"{scriptableObjectsPath}", typeof(FactoryBuilderTemplate.Item), true)
                .ForEach(this.AddDragHandles);

            // Add drag handles to items, so they can be easily dragged into the inventory if characters etc...
            tree.EnumerateTree().Where(x => x.Value as Sirenix.OdinInspector.Demos.RPGEditor.Item).ForEach(AddDragHandles);
